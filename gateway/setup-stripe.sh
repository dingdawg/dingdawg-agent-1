...
# ── Validate key (check ISG_AGENT_STRIPE_SECRET_KEY first, then bare STRIPE_SECRET_KEY) ──
if [[ -z "${ISG_AGENT_STRIPE_SECRET_KEY:-}" ]] && [[ -z "${STRIPE_SECRET_KEY:-}" ]]; then
  echo "✗ ISG_AGENT_STRIPE_SECRET_KEY is not set."
  echo "  Add it to your .env file: ISG_AGENT_STRIPE_SECRET_KEY=sk_live_..."
  echo "  (Bare STRIPE_SECRET_KEY also accepted for backward compatibility)"
  exit 1
fi

# Use ISG_AGENT_STRIPE_SECRET_KEY if set, otherwise fall back to bare STRIPE_SECRET_KEY
if [[ -n "${ISG_AGENT_STRIPE_SECRET_KEY:-}" ]]; then
  STRIPE_SECRET_KEY="$ISG_AGENT_STRIPE_SECRET_KEY"
  echo "  (using ISG_AGENT_STRIPE_SECRET_KEY)"
else
  echo "  (using bare STRIPE_SECRET_KEY — consider renaming to ISG_AGENT_STRIPE_SECRET_KEY)"
fi
...
lines = [
    "# Generated by setup-stripe.sh — copy into your .env",
    "# IMPORTANT: All vars use ISG_AGENT_ prefix because pydantic-settings",
    "# uses env_prefix='ISG_AGENT_' and silently ignores bare names.",
    "# Railway dashboard MUST show ISG_AGENT_ prefixed names — bare names",
    "# appear green but are silently ignored by pydantic-settings.",
    "",
]
for key, val in results.items():
    # Strip STRIPE_ prefix and add ISG_AGENT_STRIPE_ prefix for pydantic-settings
    if key.startswith("STRIPE_"):
        prefixed_key = f"ISG_AGENT_{key}"
    else:
        prefixed_key = f"ISG_AGENT_{key}"
    lines.append(f"{prefixed_key}={val}")
