/**
 * Hospitality industry landing page — SSR, no "use client".
 * Targets: AI agent for hotels, hotel chatbot, concierge automation.
 */
...
export const metadata: Metadata = {
  title: "AI Agent for Hospitality | DingDawg",
  description:
    "Automate room booking, concierge requests, check-in/out, and event inquiries with an AI agent built for hotels, resorts, and bed & breakfasts.",
  openGraph: {
    title: "AI Agent for Hospitality | DingDawg",
    description:
      "Delight guests with instant responses to room bookings, concierge requests, and event questions. DingDawg is the AI front desk every property needs.",
    type: "website",
  },
  keywords: [
    "AI agent for hotels",
    ...
    <div className="min-h-screen">
      ...
      <section className="px-6 pt-16 pb-12 max-w-5xl mx-auto text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[var(--gold-500)]/30 bg-[var(--gold-500)]/8 mb-8">
          <Hotel className="h-3.5 w-3.5 text-[var(--gold-500)]" />
        ...
        <h1 className="font-heading text-4xl sm:text-5xl md:text-6xl font-bold text-[var(--foreground)] leading-[1.1] tracking-tight mb-6 heading-depth">
          Your guests deserve
          <br />
          <span className="text-gradient-gold">a concierge that never sleeps.</span>
        </h1>

        <p className="text-lg sm:text-xl text-[var(--color-muted)] max-w-2xl mx-auto leading-relaxed mb-4">
          DingDawg handles room booking inquiries, concierge requests, check-in
          coordination, and event planning questions — delivering five-star responsiveness
          around the clock without adding front desk headcount.
        </p>
      ...
      <section className="px-6 pb-20 max-w-5xl mx-auto">
        <h2 className="font-heading text-2xl sm:text-3xl font-bold text-[var(--foreground)] text-center mb-3">
          Elevate guest experience from first inquiry to checkout
        </h2>
        <p className="text-center text-sm text-[var(--color-muted)] mb-10 max-w-xl mx-auto">
          Every touchpoint in the guest journey — handled professionally, instantly,
          and consistently — with zero additional staffing cost.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <UseCaseCard
            icon={<Calendar className="h-4 w-4 text-[var(--gold-500)]" />}
            title="Room Booking"
            description="Answer availability questions, explain room types, and guide guests through reservations — even when your front desk is closed."
          />
          <UseCaseCard
            icon={<Star className="h-4 w-4 text-[var(--gold-500)]" />}
            title="Concierge Requests"
            description="Handle restaurant recommendations, local activity bookings, transportation requests, and room service inquiries automatically."
          />
          <UseCaseCard
            icon={<CheckCircle className="h-4 w-4 text-[var(--gold-500)]" />}
            title="Check-In / Check-Out"
            description="Send pre-arrival instructions, collect preferences, share check-in procedures, and automate checkout confirmations via SMS and chat."
          />
          <UseCaseCard
            icon={<Users className="h-4 w-4 text-[var(--gold-500)]" />}
            title="Event Inquiries"
            description="Answer questions about wedding packages, meeting spaces, AV capabilities, and catering options. Capture leads and schedule site visits automatically."
          />
          <UseCaseCard
            icon={<Bell className="h-4 w-4 text-[var(--gold-500)]" />}
            title="Guest Notifications"
            description="Send room-ready alerts, package arrival notifications, activity reminders, and weather updates to enhance the guest stay experience."
          />
          <UseCaseCard
            icon={<MessageSquare className="h-4 w-4 text-[var(--gold-500)]" />}
            title="Post-Stay Follow-Up"
            description="Collect satisfaction feedback, request reviews, and send loyalty offers after checkout to increase repeat bookings and review scores."
          />
        </div>
      </section>
      ...
      <section className="px-6 pb-24 max-w-5xl mx-auto text-center">
        <div className="glass-panel-gold cta-gradient-border p-10 sm:p-14">
          <h2 className="font-heading text-2xl sm:text-3xl font-bold text-[var(--foreground)] mb-4 heading-depth">
            Great hospitality starts with a fast response.
            <br />
            <span className="text-gradient-gold">Yours will always be instant.</span>
          </h2>
          <p className="text-[var(--color-muted)] mb-8 max-w-md mx-auto leading-relaxed">
            50 free actions. 60-second setup. No credit card.
            <br />
            Your hospitality AI concierge is ready.
          </p>
          <Link
            href="/claim"
